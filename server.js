...
const premium = ['604041543485227009']
...
const youtube = new YouTube("AIzaSyAOGx6hDf5w5p9Ku0uf4T2mkvXVNHnYyQ8"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyAOGx6hDf5w5p9Ku0uf4T2mkvXVNHnYyQ8"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on('ready', () => {
    client.user.setActivity(`www.bestbot.ga`,{ type: 'Playing' });
})
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "settings limits")) {
    ...
    if (message.content.startsWith(prefix + "settings limitsban")) {
      ...
      if (isNaN(num)) return message.channel.send("**⇏ | only numbers ! **");
      ...
      channel.guild.members
        ...
        .catch(e =>
          channel.guild.owner.send(
            `**⇏ | ${entry.username} He wiped out a lot of rooms **`
          )
        );
      anti[channel.guild.id + entry.id].actions = "0";
  ...
  if (!anti[channel.guild.id + entry.id]) {
    ...
    if (
      ...
      channel.guild.members
        ...
        .catch(e =>
          channel.guild.owner.send(
            `**⇏ | ${entry.username} He delete many ranks **`
          )
        );
      anti[channel.guild.id + entry.id].actions = "0";
      fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
  ...
  if (!anti[channel.guild.id + entry.id]) {
    ...
    if (
      ...
      channel.guild.members
        ...
        .catch(e =>
          channel.guild.owner.send(
            `**⇏ | ${entry.username} He created many ranks **`
          )
        );
      anti[channel.guild.id + entry.id].actions = "0";
      fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
  ...
  if (!anti[guild.id + entry.id]) {
    ...
    if (anti[guild.id + entry.id].actions >= config[guild.id].banLimit) {
      guild.members
        ...
        .ban()
        .catch(e =>
          guild.owner.send(`**⇏ | ${entry.username} Try to ban all members **`)
        );
      anti[guild.id + entry.id].actions = "0";
      fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
        e
    ...
    if (!anti[member.guild.id + entry.id]) {
      ...
      if (
        ...
        member.members
          ...
          .catch(e =>
            member.owner.send(
              `**⇏ | ${entry.username} Try to ban all members**`
            )
          );
        anti[member.guild.id + entry.id].actions = "0";
        fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
...
client.on('message', message =>{
  ...
  if(!message.guild.member(message.author).hasPermissions('MANAGE_CHANNELS')) return message.reply('❌ **You do not have permission**');
  if(!message.guild.member(client.user).hasPermissions(['MANAGE_CHANNELS'])) return message.reply('❌ **The bot do not have permission**');
  message.guild.createChannel(`📊 SERVER STATS 📊:` , 'category')

    message.guild.createChannel(`wait` , 'voice').then(time => {
  ...
  setInterval(() => {
      time.setName(`👪All Members👪: ${message.guild.memberCount}`);
 },1000);
    });
...
setInterval(() => {
    time.setName(`👨Members👨: ${message.guild.members.filter(m =>!m.user.bot).size}`);
},1500);
});
